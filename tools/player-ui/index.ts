...
const timeLabel = (seconds) => {
  ...
  return (
    (hours ? hours + ":" : "") +
    String(Math.floor(value / 60) % 60).padStart(hours ? 2 : 1, "0") +
    ":" +
...
window.SteepleComponent = {
  mount(wrapper: HTMLElement, video: HTMLVideoElement) {
    const timeline = video.steepleTimeline,
      live = video.steepleIsLive;
    const controller = document.createElement("media-controller");
    controller.className = "component-player";
    for (const attr of ["noautohide", "gesturesdisabled", "nodefaultstore"])
      controller.setAttribute(attr, "");
    ...
    const listen = (element: any, type: string, callback: any, options: any = {}) =>
      element.addEventListener(type, callback, { ...options, signal: listeners.signal });
    const bar = document.createElement("media-control-bar");
    bar.className = "player-bar";
    bar.innerHTML =
      '<media-play-button class="desktop-play"></media-play-button><div class="player-volume"><media-mute-button></media-mute-button><media-volume-range></media-volume-range></div><span class="player-time"></span><media-time-range aria-label="Seek"></media-time-range><button class="player-live" type="button">Live</button><div class="player-actions"><media-captions-button></media-captions-button><media-pip-button></media-pip-button><media-fullscreen-button></media-fullscreen-button></div>';
    ...
    top.slot = "top-chrome";
    const position = bar.querySelector(".player-time")!,
      range = bar.querySelector("media-time-range")!;
    const liveButton = bar.querySelector(".player-live")!,
      actions = bar.querySelector(".player-actions")!;
    ...
    const nativeStore = createMediaStore({
      media: video,
    ...
    const project = () => {
      ...
      return {
        ...native,
        mediaCurrentTime: pendingSeek ?? state.current,
        mediaDuration: state.end,
        mediaSeekable: [state.start, state.end],
        mediaBuffered: [],
        mediaStreamType: "live",
        mediaTimeIsLive: state.live,
        mediaLoading: native.mediaLoading || state.busy,
      };
    };
    const render = () => {
      const state = project(),
        broadcast = timeline?.getState(),
        current = state.mediaCurrentTime || 0;
      actions.querySelector("media-captions-button")!.hidden = !state.mediaSubtitlesList?.length;
      position.textContent = live
        ? timeline
          ? timeLabel(current)
          : ""
        : timeLabel(current) + " / " + timeLabel(state.mediaDuration);
      if (broadcast) {
        range.toggleAttribute("disabled", !broadcast.available);
        range.title = broadcast.available
          ? "Available from " + timeLabel(broadcast.start) + " after meeting start"
          : "Rewind history is not available yet";
    ...
    const seek = (time) =>
      timeline
        ? timeline.seek(time)
        : nativeStore.dispatch({ type: "mediaseekrequest", detail: time });
    ...
    const reveal = () => {
      ...
      hideTimer = setTimeout(() => {
        if (!dragging && !video.paused && !controller.matches(":focus-within"))
          controller.setAttribute("data-hidden", "");
      }, 3000);
    };
    ...
    listen(
      ...
      "pointerdown",
      (event) => {
        setTouch(event.pointerType === "touch" || event.pointerType === "pen");
        reveal();
      },
      { capture: true },
    );
