...
test('recording, HLS and WebRTC switches reuse the video and release transports', async ({ page }) => {
  await page.goto('/');
  await startPlayback(page);
  const original = await video(page).elementHandle();
  for (const name of ['HLS', 'WebRTC', 'Recording', 'HLS', 'Recording']) {
    ...
    if (name === 'WebRTC') await expectWebRtcMedia(page);
      ...
      expect(await page.evaluate(() => window.testPeers.every(peer => peer.connectionState === 'closed'))).toBe(true);
    }
...
test('a late WHEP response cannot change a replacement recording', async ({ page }) => {
  ...
  await page.route('**/webrtc/**', async route => {
    ...
    held = true;
    await gate;
    await route.fulfill({ status: 201, headers: { location: '/webrtc/obsolete-session' }, body: 'obsolete SDP' });
  });
  await page.goto('/?mode=webrtc');
