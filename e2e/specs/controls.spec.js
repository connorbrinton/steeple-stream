...
test("recording changes and offline slates retain the video and audio preferences", async ({
  page,
}) => {
  await page.goto("/");
  ...
  await page.evaluate(() =>
    window.SteeplePlayer.renderSlate(document.querySelector("#player"), "Offline", "Waiting"),
  );
  await expect(video(page)).toHaveCount(0);
  expect(
    await original.evaluate(
      (element) => element.paused && !element.getAttribute("src") && element.srcObject === null,
    ),
  ).toBe(true);
  await page.getByRole("button", { name: "Next stream", exact: true }).click();
  await startPlayback(page);
...
test("autoplay is requested; rejection leaves a working user-initiated Play button", async ({
  page,
}) => {
  await page.addInitScript(() => {
  ...
  await page.goto("/");
  await expect(
    page.locator(".playback-status").getByRole("button", { name: /play video/i }),
  ).toBeVisible();
...
for (const width of [390, 1280])
  test(`player controls are usable at ${width}px`, async ({ page }, testInfo) => {
    ...
    await testInfo.attach(`player-${width}`, {
