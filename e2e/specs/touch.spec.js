...
test("touch overlays center playback, place actions at the top, and seek at the bottom", async ({
  page,
}, testInfo) => {
  await page.goto("/");
  await expect.poll(() => video(page).evaluate((v) => v.readyState)).toBeGreaterThanOrEqual(2);
  if (await video(page).evaluate((v) => v.paused))
    await page.locator(".playback-status button").tap();
  ...
  await expect(controller).toHaveAttribute("data-touch", "");
  await testInfo.attach("touch-recording", {
...
test("tapping hidden video reveals the overlay without pausing playback", async ({ page }) => {
  ...
  await expect(page.locator("media-controller")).toHaveAttribute("data-hidden", "", {
    timeout: 6000,
  });
  await video(page).tap({ position: { x: 20, y: 60 } });
