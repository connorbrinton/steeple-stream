...
export async function setLowVolume(page) {
  ...
  await expect
    .poll(() => video(page).evaluate((element) => element.volume > 0 && element.volume < 1))
    .toBe(true);
...
export async function expectAudio(page, volume, muted) {
  await expect
    .poll(() =>
      video(page).evaluate((element) => ({ volume: element.volume, muted: element.muted })),
    )
      ...
      () =>
        page.evaluate(async () => {
          ...
          return (
            stats.some(
              (item) =>
                item.type === "inbound-rtp" && item.kind === "video" && item.framesDecoded > 0,
            ) &&
            stats.some(
              (item) =>
                item.type === "inbound-rtp" && item.kind === "audio" && item.totalAudioEnergy > 0,
            )
          );
        }),
      { timeout: 20_000 },
