...
const generated = start('ffmpeg', ['-hide_banner', '-loglevel', 'warning', '-y',
  '-f', 'lavfi', '-i', 'testsrc2=size=640x360:rate=15',
  '-f', 'lavfi', '-i', 'sine=frequency=440:sample_rate=48000',
  '-t', '15', '-c:v', 'libx264', '-threads', '2', '-preset', 'ultrafast',
  '-pix_fmt', 'yuv420p', '-profile:v', 'baseline', '-g', '15',
  '-c:a', 'aac', '-b:a', '96k', '-movflags', '+faststart', recording], 'fixture');
const [generatedCode] = await once(generated, 'exit');
if (generatedCode !== 0) throw new Error('Could not generate recording fixture; see artifacts/fixture.log');

const mtx = start(process.env.MEDIAMTX_BINARY || 'mediamtx', [path.join(dir, 'mediamtx.yml')], 'mediamtx');
...
const publisher = start('ffmpeg', ['-hide_banner', '-loglevel', 'warning',
  '-re', '-stream_loop', '-1', '-i', recording,
  '-map', '0:v', '-map', '0:a', '-c', 'copy', '-f', 'flv', 'rtmp://127.0.0.1:1935/live',
  '-map', '0:v', '-map', '0:a', '-c:v', 'copy', '-c:a', 'libopus', '-b:a', '96k',
  '-f', 'rtsp', '-rtsp_transport', 'tcp', 'rtsp://127.0.0.1:8554/live-webrtc'], 'publisher');
for (const child of [mtx, publisher]) child.on('exit', code => {
  if (!shuttingDown) { console.error(`Media fixture exited (${code})`); process.exit(1); }
});
await waitFor(async () => {
  const response = await fetch('http://127.0.0.1:9997/v3/paths/list');
  const { items } = await response.json();
  return ['live', 'live-webrtc'].every(name => items.some(item => item.name === name && item.ready));
}, 'both audio/video streams');

server = http.createServer(async (req, res) => {
  const url = new URL(req.url, 'http://127.0.0.1:4173');
  try {
    if (url.pathname === '/ready') { res.end('ready'); return; }
    ...
    if (url.pathname.startsWith('/webrtc/')) {
      await proxyHttp(req, res, 'http://127.0.0.1:8889', req.url.slice(7), { locationPrefix: '/webrtc' });
      return;
    }
