...
async function render() {
  ...
  const options = {
    autoplay: true,
    controls: false,
    streamKey: String(generation),
    timelineStartAt: startedAt,
    timeline,
  };
  if (mode === "recording")
    window.SteeplePlayer.renderRecording(container, `/recording.mp4?stream=${generation}`, options);
  if (mode === "hls") window.SteeplePlayer.renderHls(container, "/hls/live/index.m3u8", options);
  if (mode === "webrtc")
    await window.SteeplePlayer.renderWebRtc(container, "/webrtc/live-webrtc/whep", options);
  if (mode === "hybrid")
    window.SteeplePlayer.renderHybridLive(
      container,
      {
        hlsUrl: "/hls/live/index.m3u8",
        webrtcUrl: "/webrtc/live-webrtc/whep",
      },
      options,
    );
}
