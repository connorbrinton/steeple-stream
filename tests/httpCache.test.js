...
test("frontend URLs change with contents and responses cannot be cached", async t => {
  ...
  await fs.writeFile(path.join(dir, "index.html"), '<script src="/assets/app.js?v=__ASSET_VERSION__"></script>');
  await fs.writeFile(path.join(dir, "assets/app.js"), 'window.version="one";');
  const url = await serve(t, (req, res) => sendStatic(res, dir, new URL(req.url, "http://localhost").pathname));
...
test("API responses are uncached while proxied media keeps its cache policy", async t => {
  ...
  const url = await serve(t, (req, res) => req.url === "/api/session"
    ? sendJson(res, 200, { role: "administrator" })
    : proxyHttp(req, res, upstream, "/segment"));
  assert.equal((await fetch(url + "/api/session")).headers.get("cache-control"), "private, no-store");
