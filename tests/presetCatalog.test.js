...
test("verified presets have the requested names, groups and order", () => {
  assert.deepEqual(defaultState.ptz.presets.map(({ group, name, ndiPreset }) => [group, name, ndiPreset]), [
    ["Stand", "Pulpit", 18], ["Stand", "1st Row", 12],
    ["Stand", "Stand + Congregation", 13], ["Stand", "Pulpit Wide", 2],
    ["Music", "Music Leader", 8], ["Music", "Choir", 1],
    ["Music", "Piano", 16], ["Music", "Organ", 9],
    ["Congregation", "Chapel", 4], ["Congregation", "Overflow", 6],
...
test("migration replaces old named and diagnostic presets, preserving custom entries", () => {
  ...
  const state = migrateState({ ptz: {
    presets: [
      ...["full-stand", "pulpit", "music-director", "choir", "piano", "pulpit-wide"]
        .map((id) => ({ id, ndiPreset: 2, position: { pan: 100 } })),
      ...Array.from({ length: 18 }, (_, index) => ({ id: `ndi-raw-${index + 1}`, ndiPreset: index + 1 })),
      custom
    ],
...
test("every verified preset recalls its zero-based NDI identifier without setting presets", async () => {
  ...
  const controller = new PtzController({
    ...
    runner: { async recallNdiPreset({ presetIndex }) { indices.push(presetIndex); } }
  });
  ...
  await assert.rejects(controller.recallPreset({ preset: { id: "pulpit", name: "Pulpit" }, source }), /valid NDI camera preset/);
});
