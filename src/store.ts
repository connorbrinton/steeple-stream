...
export const defaultState: ApplicationState = {
  ...
  ptz: {
    presets: [
      ...
      {
        id: "stand-congregation",
        name: "Stand + Congregation",
        group: "Stand",
        protocol: "ndi",
        ndiPreset: 13,
        position: null,
...
export function migrateState(state: unknown = {}): ApplicationState {
  ...
  migrated.cameraControlSource = migrated.cameraControlSource
    ? migrateSource(migrated.cameraControlSource)
    : null;
...
function migratePtz(value: unknown = {}): ApplicationState["ptz"] {
  ...
  const candidates =
    Array.isArray(ptz.presets) && ptz.presets.length
      ? ptz.presets.filter(isPresetCandidate)
      : defaultState.ptz.presets;
  ...
  const replaceCatalog =
    isLegacyDefaultPresetList(candidates) ||
    candidates.some(
      (preset) =>
        ["full-stand", "music-director"].includes(preset.id) || /^ndi-raw-\d+$/.test(preset.id),
    );
  // Replace the unverified catalog without carrying positions across changed camera mappings.
  const presets = replaceCatalog
    ? [
        ...structuredClone(defaultState.ptz.presets),
        ...savedPresets.filter(
          (preset) => !legacyIds.has(preset.id) && !defaultPresets.has(preset.id),
        ),
      ]
    : savedPresets;
  ...
  for (const preset of defaultState.ptz.presets) {
    if (!mergedPresets.some((entry) => entry.id === preset.id))
      mergedPresets.push(structuredClone(preset));
  }
  return {
    ...ptz,
    presets: mergedPresets,
    lastRecalledPresetId: replaceCatalog
      ? null
      : typeof ptz.lastRecalledPresetId === "string"
        ? ptz.lastRecalledPresetId
        : null,
  };
}
...
function isLegacyDefaultPresetList(presets: Array<{ id: string }>) {
  return (
    JSON.stringify(presets.map((preset) => preset.id)) ===
    JSON.stringify(["pulpit", "wide", "choir"])
  );
}
...
function migrateManualSources(
  manualSources: unknown = [],
  configuredSources: unknown = [],
): VideoSource[] {
  const sources =
    Array.isArray(manualSources) && manualSources.length ? manualSources : configuredSources;
...
function isPtzPreset(value: unknown): value is PtzPreset {
  ...
  return (
    ...
    typeof preset.ndiPreset === "number"
  );
}
