...
const mdnsPort = 5353;
export async function discoverNdiSources(currentSource) {
...
async function discoverWithNativeMdns() {
...
async function discoverWithGStreamer() {
    try {
        const stdout = await collectOutput("gst-device-monitor-1.0", ["-f", "Source/Network:application/x-ndi"], 3000);
...
export function parsePtrAnswers(buffer, queryName) {
    ...
    try {
        ...
        for (let index = 0; index < answerCount; index += 1) {
            ...
            offset += dataLength;
            if (type !== 12 || normalizeMdnsName(recordName.name) !== queryName)
                continue;
