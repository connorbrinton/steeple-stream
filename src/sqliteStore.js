...
export class SqliteStore {
    ...
    importLegacyState() {
        ...
        if (this.legacyPath && fs.existsSync(this.legacyPath)) {
            ...
            if (!fs.existsSync(backup))
                fs.copyFileSync(this.legacyPath, backup);
        }
    ...
    `).run(session.id || crypto.randomUUID(), session.viewerId, session.viewerName, session.broadcastId || null, session.locationId, session.transport || null, session.candidateType || null, session.startedAt || now, now, session.endedAt || null, Number(session.watchSeconds || 0), session.startupMs ?? null, Number(session.bufferingMs || 0), Number(session.bufferingCount || 0), Number(session.reconnectCount || 0), session.fallbackReason || null, session.terminalError || null, session.ip || null, session.userAgent || null);
    }
