...
export class BroadcastService {
    ...
    async addManualSource(source, actor = null) {
        ...
        return this.store.update((state) => {
            ...
            if (index >= 0)
                state.manualSources[index] = normalized;
            else
                state.manualSources.push(normalized);
    ...
    async capturePreset(presetId, actor = null) {
        ...
        const target = current.ptz.presets.find((entry) => entry.id === presetId);
        if (!target)
            throw Object.assign(new Error("Unknown PTZ preset"), { status: 404 });
...
function cameraControlSourceFor(state) {
    ...
    if (state.source?.type === "ndi" && state.source.ndi?.sourceName)
        return state.source;
