...
export class GStreamerMediaEngine extends EventEmitter {
    ...
    setMode(mode) {
        if (!this.process || this.currentMode === mode)
            return;
        this.currentMode = mode;
    ...
    trimLog() {
        if (this.state.log.length > 50)
            this.state.log.splice(0, this.state.log.length - 50);
    }
...
export function switchableNdiSlateToRtmpCommand({ sourceType = "ndi", networkUri, sourceName, sourceUrlAddress, rtmpUrl, webrtcRtspUrl, frameRate, videoBitrateKbps, audioBitrate, transitionDurationMs = 600, mode }) {
    ...
    if (sourceType === "ndi") {
        ...
        if (sourceUrlAddress)
            command.push("--ndi-url-address", sourceUrlAddress);
    }
    else {
        command.push("--network-uri", networkUri);
    }
    command.push("--rtmp-url", rtmpUrl, ...(webrtcRtspUrl ? ["--webrtc-rtsp-url", webrtcRtspUrl] : []), "--frame-rate", String(frameRate), "--video-bitrate-kbps", String(videoBitrateKbps), "--audio-bitrate", String(audioBitrate), "--transition-duration-ms", String(transitionDurationMs), "--initial-mode", mode);
    return command;
}
