...
export class AuthService {
    ...
    async initialize() {
        ...
        if (!this.enabled || this.config.mode === "trusted-proxy")
            return;
        this.oidc = await oidc.discovery(new URL("https://accounts.google.com"), this.config.clientId, this.config.clientSecret);
    }
    ...
    async complete(callbackUrl) {
        ...
        if (!role)
            throw httpError(403, "This Google account is not authorized for Steeple Stream");
    ...
    roleFor(email) {
        if (this.config.adminEmails.has(email))
            return "administrator";
        if (this.config.operatorEmails.has(email))
            return "operator";
        return null;
    }
    authenticate(req) {
    ...
    validateStartupConfig() {
        ...
        if (!this.config.requireProductionConfig || !isProductionAuthContext(this.config))
            return;
        const missing = [];
        if (!this.config.clientId)
            missing.push("STEEPLE_GOOGLE_CLIENT_ID");
        if (!this.config.clientSecret)
            missing.push("STEEPLE_GOOGLE_CLIENT_SECRET");
        if (!this.config.publicBaseUrl || this.config.publicBaseUrl.startsWith("http://localhost"))
            missing.push("STEEPLE_PUBLIC_BASE_URL");
        if (!this.config.adminEmails?.size)
            missing.push("STEEPLE_ADMIN_EMAILS");
        if (!this.config.operatorEmails?.size && !this.config.adminEmails?.size)
            missing.push("STEEPLE_OPERATOR_EMAILS");
        if (!isStrongSecret(this.config.sessionSecret))
            missing.push("STEEPLE_SESSION_SECRET");
        if (missing.length) {
...
export function safeReturnPath(value, channelId = "stakecenter") {
    ...
    if (parsed.origin !== "https://steeple.invalid")
        return fallback;
...
function isStrongSecret(secret) {
    if (!secret || secret === "development-only")
        return false;
