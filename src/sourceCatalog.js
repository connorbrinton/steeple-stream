export function sourceId(source) {
    if (source?.type === "ndi")
        return source.ndi?.sourceName ? `ndi:${source.ndi.sourceName}` : "";
    if (source?.type === "network")
        return source.network?.uri ? `network:${source.network.protocol || "rtsp"}:${source.network.uri}` : "";
    return "";
}
...
function sourceName(source) {
    if (source?.type === "ndi")
        return source.ndi?.sourceName || "Unnamed NDI source";
    if (source?.type === "network")
        return source.network?.uri || "Unnamed network source";
    return "Unnamed source";
}
function sourceDetail(source) {
    if (source?.type === "ndi")
        return source.ndi?.urlAddress || "NDI discovery";
    if (source?.type === "network")
...
function selectedMessage({ source, activeSource, ingestStatus, ingestReady, previewReady }) {
    if (ingestStatus?.lastError?.message)
        return ingestStatus.lastError.message;
    if (!source.available && activeSource?.type === "ndi")
        return "Selected, but not currently discovered";
    if (ingestReady && previewReady)
        return "Receiving audio, video, and preview";
    if (ingestReady)
        return "Receiving input, waiting for preview";
    if (ingestStatus?.status === "starting")
        return "Starting ingest";
    if (ingestStatus?.status === "running")
        return "Waiting for audio and video";
    return "Selected";
}
...
function bestOrigin(previous, next) {
    if (previous === "discovered" || next === "discovered")
        return "discovered";
